...
Add `-interlaced 1` to either the `mtl_st20p` demuxer or muxer to transport interlaced
video. Each ST 2110-20 frame then carries a single field, while FFmpeg keeps working on
full-height frames whose lines alternate between the first and second field. `-video_size`
and `-fps` always describe the full frame, not the field, so the wire field rate is twice
the configured `-fps`. The frame height must be even, and `yuv420p` is not supported
because its sampling is not line addressable per field.
